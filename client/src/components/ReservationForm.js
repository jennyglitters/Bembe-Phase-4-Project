...
const ReservationForm = () => {
  ...
  const { userToken, updateUserToken } = useUser();  // Combined useUser calls into one line for clarity
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    const formattedTime = values.time.slice(0, 5);  // Ensure time is in HH:MM format
    const payload = {
        ...values,
        date: formattedDate,
        time: formattedTime,  // Use the formatted time
        guest_count: parseInt(values.guests),
        menuItems: values.menuItems.map(Number),
        password: values.password && !reservationId ? values.password : undefined,
    };
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(responseData.message || `Failed to ${reservationId ? 'update' : 'create'} reservation.`);
        }

        console.log('Success response data:', responseData);
        if (responseData.token) {
            updateUserToken(responseData.token);
        }
        ...
        setSubmissionStatus({ error: error.message || 'Failed to process the form.' });
    } finally {
        setSubmitting(false);
    }
};
  ...
  const validationSchema = Yup.object().shape({
    ...
    menuItems: Yup.array()
      .of(Yup.number().positive('Invalid menu item ID'))
